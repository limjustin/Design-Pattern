...
public class Main {
    public static void main(String[] args) {
        ...
        Preprocessor preprocessor = new ConcretePreprocessor();
        Compiler compiler = new ConcreteCompiler();
        Linker linker = new ConcreteLinker();

        // 프리프로세서, 컴파일러, 링커를 각각 따로 사용해서 exe파일 생성하고 실행
        String[] fileNames = { "a.c", "b.c", "c.c" };
        // SourceCode 객체 생성
        System.out.println("소스 코드 생성");
        ...
        IDE ide = new IDE(preprocessor, compiler, linker);

        System.out.println("\nIDE에서 소스 코드 배열을 이용해서 빌드 후 실행");
        ide.createProject("ArrayProject");
        ide.build(code);

        List<SourceCode> sourceCodes = new ArrayList<>(Arrays.asList(code));

        System.out.println("\nIDE에서 소스 코드 리스트를 이용해서 빌드 후 실행");
        ide.createProject("ListProject");
        ide.build(sourceCodes);
